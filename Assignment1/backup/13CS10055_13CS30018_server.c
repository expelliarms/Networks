   ...
   while (1) {
      ...
      if (FD_ISSET(sock, &rset)) 
      {
        ...
        if(!wrongpwdflag)
        {
          ...
          write(1,buf,n);
          int i=0,j=0,k=0,deleteindexA[1000],deleteindexNA[1000];
          for(i=0;i<1000;++i)
          {
            deleteindexA[i] = 0;
            deleteindexNA[i] = 0;
          ...
          while(fgets(new_array[i],1000,fpr1) != NULL)
          {
            ...
            char thisdate[20];
            bzero(thisdate,20);
            for(k = 0; new_array[i][k]!='\0'; k++)
            {
              deletable[k] = new_array[i][k];
            }
            deletable[k-1]='\0';
            ...
            if(strcmp(thisdate,refdate)<0)
            {
              printf("thisdate =  %s\n", thisdate);
              deleteindexA[i] = 1;
              deletestaus = remove(deletable);
              if(deletestaus == 0)
                printf("%s file deleted\n", deletable);
              else
                printf("%s file not deleted. Some error\n", deletable);
            }
...
void dostuff (int sock)
{
  ...
  else if(strcmp(buffer,"Reader") == 0)
  {
    ...
    if(strcmp(buffer,"Academic") == 0)
    {
        ...
        char to_open[1000];
        to_open[0] ='\0';
        for(i = 0; i<strlen(new_array[atoi(buffer)]);i++)
        {
          to_open[i] = new_array[atoi(buffer)][i];
        }
        to_open[i-1] = '\0';
        //printf("%s\n", to_open);
